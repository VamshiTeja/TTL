...
class ConvVAE:
	def __init__(self, args, A, gamma=10):
	...
	def _create_network(self):
		...
		self.img_disc_logits, self.img_disc_prob =self.image_disc()
    ...
    def image_disc(self,x,y, is_train=True):
        with tf.variable_scope("image_disc", reuse=self.image_disc_reuse):
            ...
            disc_1 = tf.layers.conv2d(inputs=inputs, filters=64, kernel_size=3, strides=2, activation=tf.nn.leaky_relu, name="id_1", padding="same", trainable=is_train)
            # disc_1 = tf.layers.batch_normalization(disc_1,trainable=is_train)
            disc_2 = tf.layers.conv2d(inputs=disc_1, filters=64*2, kernel_size=3, strides=2, activation=tf.nn.leaky_relu, name="id_2", padding="same", trainable=is_train)
            disc_2 = tf.layers.batch_normalization(disc_2,trainable=is_train)
            disc_3 = tf.layers.conv2d(inputs=disc_2, filters=64*4, kernel_size=3, strides=2, activation=tf.nn.leaky_relu, name="id_3", padding="same", trainable=is_train)
	...
	def test_loss(self,X,Y):
		...
		return tf.reduce_mean(vae_cost).eval(), tf.reduce_mean(vae_loss_reconstr).eval()

	def theta(self,x,z,reuse=False):
		with tf.variable_scope("t_theta",reuse=reuse):
			x_ = tf.contrib.layers.flatten(x)
			z_mi=tf.layers.dense(inputs=z, units=2048,name="z_mi1",trainable=True,kernel_initializer=tf.contrib.layers.xavier_initializer())
			...
			x_mi=tf.layers.dense(inputs=x_, units=2048,name="x_mi1",trainable=True,kernel_initializer=tf.contrib.layers.xavier_initializer())
