...
class ConvVAE:
	def __init__(self, args, gamma=10):
		...
		self.x_dim = self.args.imsize
		self.y_dim = self.args.imsize
		self.imsize = self.args.imsize
		self.n_points = self.x_dim * self.y_dim

		# Define placeholders for input and output
		self.x = tf.placeholder(tf.float32, [None, self.y_dim, self.x_dim, self.args.num_channels])
		self.y = tf.placeholder(tf.float32, [None, self.y_dim, self.x_dim, self.args.num_channels])
	...
	def _encoder_network(self, inputs, is_train=True):
		...
		with tf.variable_scope("encoder"):
			e_1 = tf.layers.conv2d(inputs=inputs, filters=64, kernel_size=3, strides=2, activation=tf.nn.leaky_relu, name="e_1", padding="same", trainable=is_train)
			e_1 = tf.layers.batch_normalization(e_1,trainable=is_train)

			e_2 = tf.layers.conv2d(inputs=e_1, filters=128, kernel_size=3, strides=2, activation=tf.nn.leaky_relu, name="e_2", padding="same",trainable=is_train)
			...
			e_4 = tf.layers.conv2d(inputs=e_2, filters=256, kernel_size=3, strides=1, activation=tf.nn.leaky_relu, name="e_4", padding="same")
			e_4 = tf.layers.batch_normalization(e_4)

			e_4_reshape = tf.contrib.layers.flatten(e_4)
			e_mean = tf.layers.dense(inputs=e_4_reshape, units=self.z_dim, name="e_mean",trainable=is_train)
			e_logvar = tf.layers.dense(inputs=e_4_reshape, units=self.z_dim, name="e_logvar",trainable=is_train)
		return e_mean, e_logvar

	def _decoder_network(self,inputs, is_train=True):
		with tf.variable_scope("decoder",reuse=self.d_reuse):
			d_0 = tf.layers.dense(inputs=inputs, units=512, activation=tf.nn.relu, name="d_0",trainable=is_train)
			d_1 = tf.layers.dense(inputs=d_0, units=3136, activation=tf.nn.relu, name="d_1",trainable=is_train)
			d_1_reshape = tf.reshape(d_1, shape=[-1, 7, 7, 64])

			d_2 = tf.layers.conv2d_transpose(inputs=d_1_reshape, filters=256, kernel_size=3, strides=2, activation=tf.nn.relu, name="d_2", padding="same",trainable=is_train)
			d_2 = tf.layers.batch_normalization(d_2,trainable=is_train)

			d_3 = tf.layers.conv2d_transpose(inputs=d_2, filters=64, kernel_size=3, strides=2, activation=tf.nn.relu, name="d_3", padding="same",trainable=is_train)
			d_3 = tf.layers.batch_normalization(d_3,trainable=is_train)

			d_out = tf.layers.conv2d_transpose(inputs=d_3, filters=1, kernel_size=3, strides=1, name="d_out", padding="same",trainable=is_train)
