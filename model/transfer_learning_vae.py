...
class FactorVAE:
    def __init__(self, args, A, num_channels_x, num_channels_y, imsize, gamma=2.5, isTrain_Enc=True):
        ...
        self.decoder_saver = tf.train.Saver(var_list=tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='decoder'))
    ...
    def _create_network(self):
        ...
        self.z_mean_transfer = tf.transpose(
            tf.matmul(self.latent_selector, self.z_mean, transpose_b=True, adjoint_a=True))
        self.z_log_sigma_sq_transfer = tf.transpose(
            tf.matmul(self.latent_selector, self.z_log_sigma_sq, transpose_b=True, adjoint_a=True))
        self.z_selected = tf.add(self.z_mean_transfer,
                                 tf.multiply(tf.exp(self.z_log_sigma_sq_transfer / 2.0), self.eps))
    ...
    def _encoder_network(self, inputs, is_train=True):
        ...
        with tf.variable_scope("encoder"):
            e_1 = tf.layers.conv2d(inputs=inputs, filters=64, kernel_size=3, strides=2, activation=tf.nn.leaky_relu,
                                   name="e_1", padding="same", trainable=self.isTrain_Enc)
            e_2 = tf.layers.conv2d(inputs=e_1, filters=64 * 2, kernel_size=3, strides=2, activation=tf.nn.leaky_relu,
                                   name="e_2", padding="same", trainable=self.isTrain_Enc)
            e_4 = tf.layers.conv2d(inputs=e_2, filters=64 * 4, kernel_size=3, strides=1, activation=tf.nn.leaky_relu,
                                   name="e_4", padding="same", trainable=self.isTrain_Enc)
            e_5 = tf.layers.conv2d(inputs=e_4, filters=64 * 8, kernel_size=3, strides=1, activation=tf.nn.leaky_relu,
                                   name="e_5", padding="same", trainable=self.isTrain_Enc)
            ...
            e_logvar = tf.layers.dense(inputs=e_8_reshape, units=self.z_dim, name="e_logvar",
                                       trainable=self.isTrain_Enc)
    ...
    def _decoder_network(self, inputs, is_train=True):
        with tf.variable_scope("decoder", reuse=self.d_reuse):
            if self.num_channels_y:
                ...
                d_0 = tf.layers.dense(inputs=inputs, units=64 * 8 * s_h16 * s_w16, activation=tf.nn.relu, name="d_0",
                                      trainable=is_train)
                d_0 = tf.reshape(d_0, [-1, s_h16, s_w16, 64 * 8])
                in_shape = tf.shape(inputs)[0]
                d_1 = tf.nn.relu(tf.layers.batch_normalization(
                    deconv2d(d_0, output_shape=[in_shape, s_h8, s_w8, 64 * 4], is_train=is_train, name="d_1"),
                    trainable=is_train))
                d_2 = tf.nn.relu(tf.layers.batch_normalization(
                    deconv2d(d_1, output_shape=[in_shape, s_h4, s_w4, 64 * 2], is_train=is_train, name="d_2"),
                    trainable=is_train))
                d_3 = tf.nn.relu(tf.layers.batch_normalization(
                    deconv2d(d_2, output_shape=[in_shape, s_h2, s_w2, 64], is_train=is_train, name="d_3"),
                    trainable=is_train))
                d_out = deconv2d(d_3, output_shape=[in_shape, s_h, s_w, self.num_channels_y], is_train=is_train,
                                 name="d_4")
            else:
                d_0 = tf.nn.relu(tf.layers.batch_normalization(
                    tf.layers.dense(inputs=inputs, units=64, name="d_1", trainable=is_train), trainable=is_train))
                d_1 = tf.nn.relu(tf.layers.batch_normalization(
                    tf.layers.dense(inputs=d_0, units=64 * 2, name="d_2", trainable=is_train), trainable=is_train))
                d_2 = tf.nn.relu(tf.layers.batch_normalization(
                    tf.layers.dense(inputs=d_1, units=64 * 4, name="d_3", trainable=is_train), trainable=is_train))
                d_3 = tf.nn.relu(tf.layers.batch_normalization(
                    tf.layers.dense(inputs=d_2, units=64 * 8, name="d_4", trainable=is_train), trainable=is_train))
    ...
    def discriminator(self, z, is_train=True):
        with tf.variable_scope("discriminator", reuse=self.disc_reuse):
            disc_1 = tf.layers.dense(inputs=z, units=1024, activation=tf.nn.leaky_relu, name="disc_1",
                                     trainable=is_train)
            disc_2 = tf.layers.dense(inputs=disc_1, units=1024, activation=tf.nn.leaky_relu, name="disc_2",
                                     trainable=is_train)
            disc_3 = tf.layers.dense(inputs=disc_2, units=1024, activation=tf.nn.leaky_relu, name="disc_3",
                                     trainable=is_train)
            disc_4 = tf.layers.dense(inputs=disc_3, units=1024, activation=tf.nn.leaky_relu, name="disc_4",
                                     trainable=is_train)
            disc_5 = tf.layers.dense(inputs=disc_4, units=1024, activation=tf.nn.leaky_relu, name="disc_5",
                                     trainable=is_train)
            disc_6 = tf.layers.dense(inputs=disc_5, units=1024, activation=tf.nn.leaky_relu, name="disc_6",
                                     trainable=is_train)
    ...
    def _create_loss_optimizer(self, reconstr_loss="cross_entropy"):
        ...
        if (reconstr_loss == "cross_entropy"):
            self.reconstr_loss = tf.reduce_mean(
                tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=new_image, labels=orig_image), axis=1))
        ...
        # Kl loss
        self.vae_loss_kl = 0.5 * tf.reduce_mean(tf.reduce_sum(
            -self.z_log_sigma_sq_transfer + tf.square(self.z_mean_transfer) + tf.exp(self.z_log_sigma_sq_transfer),
            axis=1) - self.z_selected_dim)
        ...
        self.vae_optimizer = tf.train.AdamOptimizer(self.learning_rate).minimize(self.vae_cost,
                                                                                 var_list=self.enc_vars + self.dec_vars)
        self.disc_optimizer = tf.train.AdamOptimizer(self.learning_rate, beta1=0.5, beta2=0.9).minimize(self.disc_cost,
    ...
    def partial_fit_with_disc(self, X, Y):
        ...
        opt, vae_cost, vae_loss_reconstr, vae_loss_kl = self.sess.run(
            (self.vae_optimizer, self.vae_cost, self.reconstr_loss, self.vae_loss_kl),
            feed_dict={self.x: X, self.y: Y, self.eps: eps})
        opt, disc_cost, d_loss, g_loss = self.sess.run((self.disc_optimizer, self.disc_cost, self.d_loss, self.g_loss),
                                                       feed_dict={self.x: X, self.y: Y, self.eps: eps})

        return vae_cost[0], tf.reduce_mean(vae_loss_reconstr).eval(), tf.reduce_mean(
            vae_loss_kl).eval(), disc_cost, d_loss, g_loss
    ...
    def load_decoder(self, checkpoint_path):
        ckpt = tf.train.get_checkpoint_state(checkpoint_path)
        print("loading init decoder for target task: ", ckpt.model_checkpoint_path)
        self.decoder_saver.restore(self.sess, ckpt.model_checkpoint_path)

    def save_decoder(self, checkpoint_path):
    ...
    def theta(self, x, z, reuse=False):
        with tf.variable_scope("t_theta", reuse=reuse):
            ...
            z_mi = tf.layers.dense(inputs=z, units=1024, name="z_mi1", trainable=True,
                                   kernel_initializer=tf.contrib.layers.xavier_initializer())
            z_mi = tf.layers.dense(inputs=z_mi, units=512, name="z_mi2", trainable=True,
                                   kernel_initializer=tf.contrib.layers.xavier_initializer())

            x_mi = tf.layers.dense(inputs=x_, units=1024, name="x_mi1", trainable=True,
                                   kernel_initializer=tf.contrib.layers.xavier_initializer())
            x_mi = tf.layers.dense(inputs=x_mi, units=512, name="x_mi2", trainable=True,
                                   kernel_initializer=tf.contrib.layers.xavier_initializer())

            t = tf.layers.dense(inputs=z_mi + x_mi, units=1, name="t_mi", trainable=True,
                                kernel_initializer=tf.contrib.layers.xavier_initializer())
            return t
