...
def train(args):
    ...
    if args.transfer==False:
        ...
        if args.source_task=="autoencoding":
            ...
            num_channels_x= dataset_x[0].shape[-1]
            num_channels_y= num_channels_x
        target_checkpoint_dir = "./checkpoints/transfers/"+args.dataset+"/"+str(args.z_dim)+"/"+args.source_task+"->"+args.target_task+"/"+transfer_dims
        source_checkpoint_dir = "./checkpoints/source_tasks/"+args.dataset+"/"+str(args.z_dim)+"/"+args.source_task
        out_dir = "./out/transfers/"+args.dataset+"/"+str(args.z_dim)+"/"+args.source_task+"->"+args.target_task+str(args.z_dim)+"_"+transfer_dims
    ...
    if not os.path.exists(target_checkpoint_dir):
        os.makedirs(target_checkpoint_dir)
    if(args.transfer):
        vae = ConvVAE(args=args, A=A.T,num_channels_x=num_channels_x,num_channels_y=num_channels_y,imsize=imsize,isTrain_Enc=False)
    else:
        vae = ConvVAE(args=args, A=A.T,num_channels_x=num_channels_x,num_channels_y=num_channels_y,imsize=imsize,isTrain_Enc=True)
    ...
    for epoch in range(training_epochs):
		...
		save_images(recons[0:256], image_manifold_size(256),out_dir+'/epoch_{}_recons.png'.format(str(epoch+1)))
	    else:
